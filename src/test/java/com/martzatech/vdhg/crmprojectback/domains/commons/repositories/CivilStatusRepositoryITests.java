...
class CivilStatusRepositoryITests {

  private static final AtomicInteger ID_WRAPPER = new AtomicInteger();
  private static final String NEW_CIVIL_STATUS = "NEW_CIVIL_STATUS";
  private static final String OTHER_CIVIL_STATUS = "OTHER_CIVIL_STATUS";

  @Autowired
  private CivilStatusRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final CivilStatusEntity entity = new CivilStatusEntity(null, NEW_CIVIL_STATUS);

    final CivilStatusEntity actual = tested.save(entity);

    Assertions.assertNotNull(actual);
    Assertions.assertNotNull(actual.getId());
    Assertions.assertEquals(NEW_CIVIL_STATUS, actual.getName());
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<CivilStatusEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final CivilStatusEntity existing = new CivilStatusEntity(ID_WRAPPER.get(), OTHER_CIVIL_STATUS);

    final CivilStatusEntity actual = tested.save(existing);

    Assertions.assertEquals(ID_WRAPPER.get(), actual.getId());
    Assertions.assertEquals(OTHER_CIVIL_STATUS, actual.getName());
  }
