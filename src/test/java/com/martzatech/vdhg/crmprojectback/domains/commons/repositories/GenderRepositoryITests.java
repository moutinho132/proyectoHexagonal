...
class GenderRepositoryITests {
  ...
  @Autowired
  private GenderRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final GenderEntity entity = new GenderEntity(null, NAME);

    final GenderEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<GenderEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final GenderEntity existing = new GenderEntity(ID_WRAPPER.get(), OTHER_NAME);

    final GenderEntity actual = tested.save(existing);
