...
class IdentityDocumentTypeRepositoryITests {
  ...
  @Autowired
  private IdentityDocumentTypeRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final IdentityDocumentTypeEntity entity = new IdentityDocumentTypeEntity(null, NAME);

    final IdentityDocumentTypeEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<IdentityDocumentTypeEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final IdentityDocumentTypeEntity existing = new IdentityDocumentTypeEntity(ID_WRAPPER.get(), OTHER_NAME);

    final IdentityDocumentTypeEntity actual = tested.save(existing);
