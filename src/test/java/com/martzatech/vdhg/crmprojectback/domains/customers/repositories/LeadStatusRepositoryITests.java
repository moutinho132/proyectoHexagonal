...
class LeadStatusRepositoryITests {
  ...
  @Autowired
  private LeadStatusRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final LeadStatusEntity entity = LeadStatusEntity.builder().name(NAME).build();

    final LeadStatusEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<LeadStatusEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final LeadStatusEntity existing = LeadStatusEntity.builder().id(ID_WRAPPER.get()).name(OTHER_NAME).build();

    final LeadStatusEntity actual = tested.save(existing);
