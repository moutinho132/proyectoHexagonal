...
class EmployeeTypeRepositoryITests {
  ...
  @Autowired
  private EmployeeTypeRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final EmployeeTypeEntity entity = EmployeeTypeEntity.builder().name(NAME).build();

    final EmployeeTypeEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<EmployeeTypeEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final EmployeeTypeEntity existing = EmployeeTypeEntity.builder().id(ID_WRAPPER.get()).name(OTHER_NAME).build();

    final EmployeeTypeEntity actual = tested.save(existing);
