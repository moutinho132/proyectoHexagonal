...
class SubCategoryRepositoryITests {
  ...
  @Autowired
  private SubCategoryRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final SubCategoryEntity entity = SubCategoryEntity.builder().name(NAME).build();

    final SubCategoryEntity actual = tested.save(entity);
...
/*
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<SubCategoryEntity> actual = tested.findById(ID_WRAPPER.get());
...
*/
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final SubCategoryEntity existing = SubCategoryEntity.builder().id(ID_WRAPPER.get()).name(OTHER_NAME).build();

    final SubCategoryEntity actual = tested.save(existing);
