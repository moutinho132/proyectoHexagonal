...
class PhoneRepositoryITests {
  ...
  @Autowired
  private PhoneRepository tested;
  ...
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    ...
    final PhoneEntity entity = PhoneEntity.builder().value(VALUE).valid(VALID).checkTime(CHECK_TIME)
        .attributeType(attributeTypeEntity).build();

    final PhoneEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<PhoneEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    ...
    final PhoneEntity existing = PhoneEntity.builder().id(ID_WRAPPER.get()).value(OTHER_VALUE).valid(OTHER_VALID)
        .checkTime(OTHER_CHECK_TIME).attributeType(otherAttributeTypeEntity).build();

    final PhoneEntity actual = tested.save(existing);
