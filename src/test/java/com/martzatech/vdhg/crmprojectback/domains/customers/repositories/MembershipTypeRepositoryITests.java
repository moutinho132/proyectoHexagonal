...
class MembershipTypeRepositoryITests {
  ...
  @Autowired
  private MembershipTypeRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final MembershipTypeEntity entity = MembershipTypeEntity.builder().name(NAME).build();

    final MembershipTypeEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<MembershipTypeEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final MembershipTypeEntity existing = MembershipTypeEntity.builder().id(ID_WRAPPER.get()).name(OTHER_NAME).build();

    final MembershipTypeEntity actual = tested.save(existing);
