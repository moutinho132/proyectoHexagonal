...
class CustomerStatusRepositoryITests {
  ...
  @Autowired
  private CustomerStatusRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final CustomerStatusEntity entity = CustomerStatusEntity.builder().name(NAME).build();

    final CustomerStatusEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<CustomerStatusEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final CustomerStatusEntity existing = CustomerStatusEntity.builder().id(ID_WRAPPER.get()).name(OTHER_NAME).build();

    final CustomerStatusEntity actual = tested.save(existing);
