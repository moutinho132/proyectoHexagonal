...
class AttributeTypeRepositoryITests {
  ...
  @Autowired
  private AttributeTypeRepository tested;

  @Order(1)
  @Test
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final AttributeTypeEntity entity = new AttributeTypeEntity(null, NAME);

    final AttributeTypeEntity actual = tested.save(entity);
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<AttributeTypeEntity> actual = tested.findById(ID_WRAPPER.get());
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final AttributeTypeEntity existing = new AttributeTypeEntity(ID_WRAPPER.get(), OTHER_NAME);

    final AttributeTypeEntity actual = tested.save(existing);
