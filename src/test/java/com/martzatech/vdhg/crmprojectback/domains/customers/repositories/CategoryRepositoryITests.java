...
class CategoryRepositoryITests {
  ...
  @Autowired
  private CategoryRepository tested;
  ...
  void givenANewObject_whenSave_thenNewObjectIsPersisted() {
    final CategoryEntity entity = CategoryEntity.builder().name(NAME).build();

    final CategoryEntity actual = tested.save(entity);
...
/*
  ...
  void whenFindById_thenReturnPersistedObject() {
    final Optional<CategoryEntity> actual = tested.findById(ID_WRAPPER.get());
...
/*
  ...
  void givenAExistingObject_whenSave_thenUpdateObject() {
    final CategoryEntity existing = CategoryEntity.builder()
        .id(ID_WRAPPER.get())
        .name(OTHER_NAME)
        .subCategories(new ArrayList<>())
        .build();

    final CategoryEntity actual = tested.save(existing);
